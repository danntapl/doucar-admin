...
/*
**Note:** We are reading `joined_all` rather than `joined`.  We developed our
Pipeline and PipelineModel on `joined` and we will apply them to `joined_all`.
*/
...
val pipelineModel = PipelineModel.load("myduocar/pipeline_model")
...
val pipeline = Pipeline.load("myduocar/pipeline")
...
```scala
spark.stop()  // Is this needed in a Scala session?
```
*/
